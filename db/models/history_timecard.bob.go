...
// historyTimecardR is where relationships are stored.
type historyTimecardR struct {
	Organization *Organization // history_timecard.history_timecard_organization_id_fkey
}
...
func attachHistoryTimecardOrganization0(ctx context.Context, exec bob.Executor, count int, historyTimecard0 *HistoryTimecard, organization1 *Organization) (*HistoryTimecard, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("attachHistoryTimecardOrganization0: %w", err)
	}

	return historyTimecard0, nil
}
...
func (historyTimecard0 *HistoryTimecard) InsertOrganization(ctx context.Context, exec bob.Executor, related *OrganizationSetter) error {
	...
	_, err = attachHistoryTimecardOrganization0(ctx, exec, 1, historyTimecard0, organization1)
	if err != nil {
		return err
	}

	historyTimecard0.R.Organization = organization1

	organization1.R.HistoryTimecards = append(organization1.R.HistoryTimecards, historyTimecard0)
...
func (o *HistoryTimecard) Preload(name string, retrieved any) error {
	...
	case "Organization":
		...
		if !ok {
			return fmt.Errorf("historyTimecard cannot load %T as %q", retrieved, name)
		}

		o.R.Organization = rel

		if rel != nil {
			rel.R.HistoryTimecards = HistoryTimecardSlice{o}
		}
		return nil
	default:
		return fmt.Errorf("historyTimecard has no relationship %q", name)
	}
}

type historyTimecardPreloader struct {
	Organization func(...psql.PreloadOption) psql.Preloader
}

func buildHistoryTimecardPreloader() historyTimecardPreloader {
	return historyTimecardPreloader{
		Organization: func(opts ...psql.PreloadOption) psql.Preloader {
			return psql.Preload[*Organization, OrganizationSlice](psql.PreloadRel{
				Name: "Organization",
				Sides: []psql.PreloadSide{
					{
						From:        HistoryTimecards,
						To:          Organizations,
...
type historyTimecardThenLoader[Q orm.Loadable] struct {
	Organization func(...bob.Mod[*dialect.SelectQuery]) orm.Loader[Q]
}

func buildHistoryTimecardThenLoader[Q orm.Loadable]() historyTimecardThenLoader[Q] {
	type OrganizationLoadInterface interface {
		LoadOrganization(context.Context, bob.Executor, ...bob.Mod[*dialect.SelectQuery]) error
	}

	return historyTimecardThenLoader[Q]{
...
func (o *HistoryTimecard) LoadOrganization(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	related.R.HistoryTimecards = HistoryTimecardSlice{o}
...
type historyTimecardJoins[Q dialect.Joinable] struct {
	typ          string
	Organization modAs[Q, organizationColumns]
}

func (j historyTimecardJoins[Q]) aliasedAs(alias string) historyTimecardJoins[Q] {
	return buildHistoryTimecardJoins[Q](buildHistoryTimecardColumns(alias), j.typ)
}

func buildHistoryTimecardJoins[Q dialect.Joinable](cols historyTimecardColumns, typ string) historyTimecardJoins[Q] {
	return historyTimecardJoins[Q]{
		typ: typ,
		Organization: modAs[Q, organizationColumns]{
